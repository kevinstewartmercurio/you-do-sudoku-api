...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
    try {
        ...
        if (req.body.difficulty) {
            if (req.body.difficulty === "easy" || req.body.difficulty === "medium" || req.body.difficulty === "hard") {
                ...
                return res.status(400).json({
                    error: "Invalid 'difficulty' input. Please provide 'easy', 'medium', or 'hard', or use the default difficulty of 'easy'."
                })
            }
        } else {
        ...
        let retObj: {[key: string]: any} = {difficulty: req.body.difficulty || "easy"}
        ...
        const [puzzleIsometry, solutionIsometry] = getIsometry(boardObj.sudoku.split(",")[0], boardObj.sudoku.split(",")[1])
        ...
        if (req.body.solution && typeof(req.body.solution) !== "boolean") {
            return res.status(400).json({
                error: "Invalid 'solution' input. Please provide a boolean value or use the default value of false."
            })
